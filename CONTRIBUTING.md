...
#### Issues
Issue labels are prefixed by `Issue:`.
- `Issue: Bug` - Applied to issues reporting PocketMine-MP bugs.
- `Issue: Enhancement` - Applied to issues containing feature requests or ToDo lists.
...
*Generic*
- `Status: High Priority` - applied when the bug is easy to fix, or if the scale of the bug is global. This label may also be applied to PRs which fix critical bugs.

#### Resolution
Resolution labels show the resolution of an issue. These labels may only be applied to closed issues.
